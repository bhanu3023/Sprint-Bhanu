...
async function runMigrations(pool, opts = {}) {
  ...
  try {
    ...
    for (const m of pending) {
      ...
      // `pool` is passed alongside `client` so a migration needing a SECOND,
      // non-transactional connection (CREATE INDEX CONCURRENTLY cannot run
      // inside client's open transaction) requests one from the SAME pool
      // this run was invoked with, rather than reaching for the app's global
      // db.js singleton -- which is only the same pool by coincidence in
      // production, and is a DIFFERENT database entirely in any test that
      // points runMigrations at a database other than DATABASE_URL.
      try {
        await client.query('BEGIN');
        const detail = await m.up(client, pool);
