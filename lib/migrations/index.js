...
const MIGRATIONS = [
  ...
  {
    id: '017b-dedupe-issue-keys-active-space-priority',
    description: 'Resolve duplicate issues.key values BEFORE 018 tries to add the UNIQUE constraint. Root cause: archiving a space is a soft-delete (spaces.is_archived=true) -- the row and its issues stay put. A NEW space can reuse that same key and starts its own issue_counter at 1, colliding with the archived space\'s still-live issue keys. Priority: the row belonging to the currently ACTIVE (non-archived) space keeps its key untouched; the row belonging to the ARCHIVED space is renamed to the next free key in ITS OWN space (bumping that space\'s issue_counter so future creates never collide with it). Ties, or a duplicate group with no active-space row at all, fall back to earliest created_at. Never merges or deletes a row -- only issues.key changes.',
    async up(client) {
      const dupes = (await client.query(`
        SELECT i.key AS dupkey,
          array_agg(i.id ORDER BY sp.is_archived ASC, i.created_at ASC, i.id ASC) AS ids
        FROM issues i
      ...
      if (!dupes.length) return 'no duplicate issue keys -- nothing to resolve';

      const renamed = [];
      for (const dup of dupes) {
        // ids[0] is the keeper: active-space row wins, ties broken by earliest
        // created_at. Every other id in the group gets a fresh key.
        for (let i = 1; i < dup.ids.length; i++) {
          const rowId = dup.ids[i];
          const spaceRow = (await client.query('SELECT space_id FROM issues WHERE id=$1', [rowId])).rows[0];
          const spaceKeyRow = (await client.query('SELECT key FROM spaces WHERE id=$1', [spaceRow.space_id])).rows[0];

          let newKey;
          for (;;) {
            const counterRow = (await client.query(
              'UPDATE spaces SET issue_counter = issue_counter + 1 WHERE id=$1 RETURNING issue_counter',
              [spaceRow.space_id]
            )).rows[0];
            newKey = spaceKeyRow.key + '-' + counterRow.issue_counter;
            const clash = await client.query('SELECT 1 FROM issues WHERE key=$1', [newKey]);
            if (!clash.rows.length) break;
          }

          await client.query('UPDATE issues SET key=$1 WHERE id=$2', [newKey, rowId]);
          renamed.push(rowId + ' (' + dup.dupkey + ' -> ' + newKey + ')');
        }
      }
      return renamed.length + ' issue(s) renamed to resolve key collisions: ' + renamed.join('; ');
    }
  },
  {
  ...
  {
    id: '020-soft-delete-spaceless-issues',
    description: 'db/init.sql declares a FK from issues.space_id to spaces(id) with no ON DELETE, and the app only ever soft-archives a space (is_archived=true) through DELETE /api/spaces/:id -- so under the SCHEMA AS WRITTEN, a dangling reference to a hard-removed space should not be reachable. But issues_key_key was proof this production database has drifted from init.sql before (undocumented manual patch, absent from every migration), so this checks for BOTH ways an issue can end up "not related to any space" rather than trusting the FK is actually present on the live schema: space_id IS NULL, and space_id pointing at a spaces.id that no longer exists at all (which is only possible if the FK is missing in prod, or was bypassed). Either way, such rows are invisible to every normal view (GET /api/issues requires space_id; the space-scoped GET /api/data path filters by it) and only ever surfaced through the unscoped admin bulk load. This soft-deletes them the same way DELETE /api/issues/:id already does -- deleted_at=NOW(), nothing else touched -- so an org admin can still find and restore any of them from the Deleted Items bin if one turns out to matter. Never a hard DELETE, and issues belonging to an ARCHIVED-but-still-existing space are correctly left alone -- archived is not "not related to any space".',
    async up(client) {
      const orphans = (await client.query(`
        SELECT i.id, i.key, i.space_id
        FROM issues i
        WHERE i.deleted_at IS NULL
          AND (i.space_id IS NULL OR NOT EXISTS (SELECT 1 FROM spaces sp WHERE sp.id = i.space_id))
        ORDER BY i.created_at LIMIT 20
      `)).rows;
      if (!orphans.length) return 'no spaceless/orphaned issues -- nothing to bin';

      const r = await client.query(`
        UPDATE issues SET deleted_at=NOW()
        WHERE deleted_at IS NULL
          AND (space_id IS NULL OR NOT EXISTS (SELECT 1 FROM spaces sp WHERE sp.id = issues.space_id))
      `);
      return r.rowCount + ' spaceless/orphaned issue(s) soft-deleted (restorable from the Deleted Items bin). Sample: ' +
        orphans.map(function (o) { return (o.key || o.id) + (o.space_id ? ' [dangling space_id ' + o.space_id + ']' : ' [NULL space_id]'); }).join(', ') +
        (r.rowCount > orphans.length ? ', ... (' + (r.rowCount - orphans.length) + ' more)' : '');
    }
  },
  {
    id: '021-dedupe-and-enforce-space-key-uniqueness',
    description: 'Migration 014 deliberately does not fail the boot when spaces.key already has duplicates -- it logs a warning, skips creating idx_spaces_key_unique, and still records itself as applied. That means once a duplicate existed at boot time, the DB-level unique index NEVER gets created, even after the duplicate is later resolved -- 014 will not run again. This is exactly the same shape of gap that produced the issues.key incident (a space with key "AI" was archived, another space later reused "AI", and their issues collided), and it reproduces independently: even this local dev database has spaces.key "DEM" duplicated the same way (one archived, one active). This migration (a) resolves any existing spaces.key duplicate the same way 017b resolves issue-key duplicates -- the ACTIVE space keeps its key, the ARCHIVED space is renamed to "<key>-ARCHIVED" (or "<key>-ARCHIVED-2", etc. if that also collides); ties/no-active-row groups fall back to earliest created_at -- then (b) creates idx_spaces_key_unique itself if 014 never managed to, closing the gap for good. POST /api/spaces already blocks reusing an archived space\'s key at the app layer (findSpaceKeyConflict in spaces.js), but that is a SELECT-then-INSERT check, not atomic -- two concurrent creates could still race past it. The DB-level unique index is what actually makes a repeat of this incident structurally impossible, not just unlikely.',
    async up(client) {
      const messages = [];

      const dupeGroups = (await client.query(`
        SELECT UPPER(key) AS k,
          array_agg(id ORDER BY is_archived ASC, created_at ASC, id ASC) AS ids
        FROM spaces
      ...
      if (dupeGroups.length) {
        ...
        for (const grp of dupeGroups) {
          // ids[0] is the keeper (active space wins, tie -> earliest created_at).
          for (let i = 1; i < grp.ids.length; i++) {
            const rowId = grp.ids[i];
            const origKeyRow = (await client.query('SELECT key FROM spaces WHERE id=$1', [rowId])).rows[0];
            const origKey = origKeyRow.key;

            let newKey = origKey + '-ARCHIVED';
            let n = 2;
            for (;;) {
              const clash = await client.query('SELECT 1 FROM spaces WHERE UPPER(key)=UPPER($1)', [newKey]);
              if (!clash.rows.length) break;
              newKey = origKey + '-ARCHIVED-' + n;
              n++;
            }

            await client.query('UPDATE spaces SET key=$1, updated_at=NOW() WHERE id=$2', [newKey, rowId]);
            renamed.push(rowId + ' (' + origKey + ' -> ' + newKey + ')');
          }
        }
        messages.push(renamed.length + ' duplicate space key(s) resolved: ' + renamed.join('; '));
      } else {
        messages.push('no duplicate space keys');
      }

      const already = await client.query(
        "SELECT 1 FROM pg_indexes WHERE tablename='spaces' AND indexname='idx_spaces_key_unique'"
      );
      if (already.rows.length) {
        messages.push('idx_spaces_key_unique already present');
      } else {
        await client.query('CREATE UNIQUE INDEX idx_spaces_key_unique ON spaces (UPPER(key))');
        messages.push('idx_spaces_key_unique created');
      }
  ...
  {
    id: '022-combination-upgraders',
    description: 'Per-combination "Upgrader" assignment (the person who handles a given Source-Destination combination) -- one row per (field, combination string), so an admin can assign a specific person to each combination the space has configured, independent of the combination options themselves.',
    async up(client) {
      await client.query(`CREATE TABLE IF NOT EXISTS combination_upgraders (
        id VARCHAR PRIMARY KEY,
      ...
      await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_combination_upgraders_field_combo
        ON combination_upgraders (field_id, combination)`);
      await client.query(`CREATE INDEX IF NOT EXISTS idx_combination_upgraders_user
        ON combination_upgraders (user_id)`);
    }
  },
  {
    id: '023-issue-drafts',
    description: 'Autosaved Create Issue drafts -- whatever a user has typed into the Create Issue form before actually submitting it, so a refresh, an accidental close, or just walking away mid-form does not lose it. Strictly personal: no space-permission gate, only ownership (user_id = the reader/writer), since a draft is not yet a real issue and never touches issue_history, notifications, or a space\'s issue_counter.',
    async up(client) {
      await client.query(`CREATE TABLE IF NOT EXISTS issue_drafts (
        id VARCHAR PRIMARY KEY,
        user_id VARCHAR NOT NULL REFERENCES users(id) ON DELETE CASCADE,
        space_id VARCHAR REFERENCES spaces(id) ON DELETE SET NULL,
      ...
      await client.query(`CREATE INDEX IF NOT EXISTS idx_issue_drafts_user
        ON issue_drafts (user_id, updated_at DESC)`);
    }
  },
  {
    id: '024-combination-upgrader-roles',
    description: 'A combination can now have more than one Upgrader, one per "role" (Frontend/Backend today, but the role LIST itself is admin-configurable per field, not hardcoded, so a space can add e.g. a QA role later with no code change). combination_upgraders gains a role column (unique key becomes field_id+combination+role instead of field_id+combination); every row that already existed is backfilled as role=\'backend\' -- the existing single-Upgrader feature this session already shipped is treated as "the Backend Upgrader" going forward, per instruction, not discarded. A new combination_upgrader_roles table holds the configurable role list per field, seeded with Frontend/Backend for every Combination field that exists today so the admin UI has the same two roles to start from. custom_fields gains upgrader_role_required_types (VARCHAR[]), mirroring the existing required_types column exactly (see 009-custom-fields-required-types) but as its own separate, opt-in setting: NULL/empty means role selection is never required (the safe default -- this is a brand new concept, so no existing space should suddenly gain a new required field it never configured), a non-empty list names the issue types for which picking a role becomes mandatory.',
    async up(client) {
      await client.query(`CREATE TABLE IF NOT EXISTS combination_upgrader_roles (
        id VARCHAR PRIMARY KEY,
        field_id VARCHAR NOT NULL REFERENCES custom_fields(id) ON DELETE CASCADE,
      ...
      await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_combination_upgrader_roles_field_key
        ON combination_upgrader_roles (field_id, key)`);

      const alreadyHasRole = await client.query(
        "SELECT 1 FROM information_schema.columns WHERE table_name='combination_upgraders' AND column_name='role'"
      );
      if (!alreadyHasRole.rows.length) {
        await client.query(`ALTER TABLE combination_upgraders ADD COLUMN role VARCHAR NOT NULL DEFAULT 'backend'`);
      }
      await client.query('DROP INDEX IF EXISTS idx_combination_upgraders_field_combo');
      await client.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_combination_upgraders_field_combo_role
        ON combination_upgraders (field_id, combination, role)`);

      await client.query(`ALTER TABLE custom_fields ADD COLUMN IF NOT EXISTS upgrader_role_required_types VARCHAR[]`);

      // Seed Frontend/Backend for every Combination field that already
      // exists, so the admin UI has real rows to show and edit immediately
      // instead of an empty role list on every space that already uses
      // Combination + Upgraders today.
      const combinationFields = (await client.query(
        `SELECT id FROM custom_fields WHERE field_key='combination' OR LOWER(name)='combination'`
      )).rows;
      for (const field of combinationFields) {
        for (const role of [{ name: 'Frontend', key: 'frontend', position: 0 }, { name: 'Backend', key: 'backend', position: 1 }]) {
          await client.query(
            `INSERT INTO combination_upgrader_roles(id, field_id, name, key, position)
             VALUES($1,$2,$3,$4,$5) ON CONFLICT (field_id, key) DO NOTHING`,
            [uid(), field.id, role.name, role.key, role.position]
          );
        }
      }
    }
  ...
  {
    id: '025-issue-attachments-index',
    description: 'Adds an index on issue_attachments.issue_id. Postgres does not automatically index a foreign-key column, and GET /api/issues/:id queries this table with WHERE issue_id=$1 on every single ticket open -- with no index that was a sequential scan over the WHOLE table, whose cost grows with the total number of attachments across every ticket in the system, not just this one. Invisible on a small local seed dataset; measurable in production once real attachment volume accumulates. Purely additive -- CREATE INDEX IF NOT EXISTS -- and changes no query result, only how fast it is found.',
    async up(client) {
      await client.query(`CREATE INDEX IF NOT EXISTS idx_issue_attachments_issue_id ON issue_attachments (issue_id)`);
    }
  },
  {
    id: '026-issue-links-target-id-index',
    description: 'Adds an index on issue_links.target_id. The existing UNIQUE(source_id, target_id, link_type) constraint (schema.sql) only helps a lookup that STARTS with source_id -- a composite index cannot serve a search on its second column alone. GET /api/issues/:id (and every other issue_links reader) queries WHERE (source_id=$1 OR target_id=$1), since a link is meant to be found from either end -- so the target_id half of that OR, on every single ticket that is the TARGET of a link rather than the source, had no index to use at all. Purely additive, changes no query result.',
    async up(client) {
      await client.query(`CREATE INDEX IF NOT EXISTS idx_issue_links_target_id ON issue_links (target_id)`);
    }
  }
];
