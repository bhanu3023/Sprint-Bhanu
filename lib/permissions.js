...
const ACTION_DENIED_MESSAGE = {
  ...
  'comment.delete': 'You can only delete your own comments.',
...
const UPDATE_WHITELIST = {
  ...
  // 'status' and 'velocity' are deliberately NOT here. They belong to the
  // lifecycle endpoints (/start, /complete) which own the ordered side effects
  // -- velocity calculation, moving incomplete issues to the backlog, member
  // notifications. Writable through this generic PUT they were a second,
  // unguarded door: a site_admin could set status='completed' with none of
  // that happening, or reactivate a sprint after A4 closed /start. Verified no
  // client sends either -- src/client/crud/sprint.js:7 is the only PUT payload
  // and carries name, goal, start_date, end_date, developer_ids, qa_ids,
  // public_holidays, developer_leaves.
  sprints: ['name', 'goal', 'start_date', 'end_date', 'position', 'developer_ids', 'qa_ids', 'public_holidays', 'developer_leaves', 'achievements'],
  issues: [
  ...
  custom_fields: ['name', 'field_type', 'options', 'is_required', 'position', 'show_in', 'required_types', 'upgrader_role_required_types'],
...
// `knownRole` is a PERFORMANCE-ONLY escape hatch: a caller that already has
// the actor's normalized space role in hand (e.g. from a LEFT JOIN on its own
// main query) can pass it here to skip the extra `getSpaceMemberRole` round
// trip entirely. Every existing caller omits it (stays `undefined`) and gets
// the exact original behavior -- the query still runs. Pass `null` (not
// `undefined`) to assert "confirmed not a member" without a query.
async function canActInSpace(q, user, spaceId, action, knownRole) {
  if (!user || !spaceId) return false;
  if (isOrgAdmin(user.role)) return true;
  const minRole = ACTION_MIN_ROLE[action];
  if (!minRole) return false;
  const userId = user.id || user.user_id;
  const spaceRole = knownRole !== undefined ? knownRole : await getSpaceMemberRole(q, userId, spaceId);
 ...
 * `knownRole`: see canActInSpace's own comment — optional perf escape hatch. */
async function denyUnlessCanAct(q, user, res, spaceId, action, knownRole) {
  if (!spaceId) {
    res.status(400).json({ error: 'Space context required' });
    return false;
  }
  const ok = await canActInSpace(q, user, spaceId, action, knownRole);
  if (!ok) {
