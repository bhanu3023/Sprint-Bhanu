...
|---|---|---|---|
| 1 | **Move purity** | `node scripts/refactor-verify/catdiff.js` | Four sub-checks, all on **RAW untransformed bytes**: **[A]** each part's bytes equal the original's bytes for the line range it claims; **[B]** the ranges tile 1..N with no gap or overlap; **[C]** concatenating them reproduces the whole original byte-for-byte — or, when  are declared, **reversibility**: un-applying every declared move against the files' actual bytes reproduces the original line-for-line; **[D]** the `<script src>` order parsed from the *real* `index.html` equals the parts' ascending line order. Order is never trusted from the manifest — a disagreement is reported as the bug. |
